...
const clockInButton = element(
  by.cssContainingText('button', 'Clock In')
);
const clockOutButton = element(
  by.cssContainingText('button', 'Clock Out')
);
...
describe(appPath, async () => {
  ...
  it('should hover over schedule', async () => {
    await browser
      ...
      .perform();
    await browser.sleep(300); // Hover is not instant
